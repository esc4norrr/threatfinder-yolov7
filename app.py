...
p_time = 0
st.set_page_config(page_title="ThreatFinder")
st.sidebar.title('Settings')
# Choose the model
model_type = st.sidebar.selectbox(
    'Choose YOLO Model', ('YOLO Model','YOLOv7')
)
    ...
    if st.sidebar.checkbox('Load Model'):
        ...
        if model_type == 'YOLOv7':
            ...
            if not torch.cuda.is_available():
                st.sidebar.warning('CUDA Not Available, choose CPU')
            else:
                st.sidebar.success(
...
if (cap != None) and pred:
    ...
    while True:
        ...
        class_fq = json.dumps(class_fq, indent = 4)
        class_fq = json.loads(class_fq)
        sus_classes = [1, 2, 3, 6, 7,9,12,16] # threatening classes
        sus_classes_present = any(elem in current_no_class for elem in sus_classes)
        if sus_classes_present:
